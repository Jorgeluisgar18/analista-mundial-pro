...
|-------|-----|--------|-----------|----------------|
| 2026-07-14 | Codex | Cierre de QA real/export: smoke local de `/api/health`, búsquedas por Mundial/Premier/Champions/LaLiga/fecha vacía, export HTML local y producción solo lectura. Export HTML ampliado con cancha táctica simplificada, bajas por tipo, calidad por proveedor y resumen de cuotas por casa/mercado. Se añadió reporte `docs/qa/qa-real-providers-2026-07-14.md`. | **RED/GREEN:** `export-html` falló primero por ausencia de cancha/bajas/calidad/cuotas y luego pasó; E2E de descarga valida los nuevos bloques. **QA local:** DB conectada, telemetría conectada, Elo 100%, backtesting listo con 817 muestras; API-Football 1/100 día, Footballdata.io 82/1000 mes, Odds API 11/500 mes, TheSportsDB configurado; Football-Data.org implementado pero sin `FOOTBALL_DATA_API_KEY` activa en `.env.local`. **Producción:** URL conocida `shiny-torte-4f01e2.netlify.app` responde 404 en `/api/health` y export, por lo que queda pendiente confirmar URL/deploy. **Verificación:** `npx tsc --noEmit` OK; `npm run lint` OK; `git diff --check` OK; `npm test` 57 pass/5 skipped, 202 pass/8 skipped; `npm run test:e2e` 11 pass/1 skipped; `npm run build` OK; secret scan focal sin hallazgos. | Commit/push autorizado por owner. Pendiente real: configurar `FOOTBALL_DATA_API_KEY` local/Netlify y ejecutar smoke post-deploy cuando Netlify vuelva a tener builds/créditos. |
| 2026-07-14 | Codex | Export HTML alineado con cabina: `renderAnalysisHtml` ahora acepta `dataset` opcional y la ruta `/api/match/[id]/export` lo pasa desde `getAnalysis`. El informe descargado incluye `Alineaciones y disponibilidad`, `Jugadores clave` y `Contexto histórico` con XI esperado/oficial, fuente de formación, titulares, proyecciones individuales, Elo y forma histórica ponderada/ajustada por rival. | **RED/GREEN:** `tests/unit/export-html.test.ts` falló primero por ausencia de esos bloques y luego pasó. **E2E focal:** descarga HTML autónoma valida los nuevos bloques en el archivo real. Sin commit ni push aún en esta sesión por falta de autorización explícita nueva. | Ejecutar verificación completa y, si owner autoriza, commit/push del bloque. Siguiente recomendado: QA real por proveedores/fechas o exportar cancha visual simplificada en HTML. |
