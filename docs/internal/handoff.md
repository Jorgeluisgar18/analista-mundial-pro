...
|---|---|---|---|---|
| 2026-07-30 | Codex | Eliminación de bloqueo interno de ligas futuras y configuración de respaldo europeo. | API-Football ya no omite Premier, LaLiga o Champions solo por ser posteriores a 2024: cada consulta pasa por la protección central de cuota/caché y deja que el proveedor responda su cobertura real. Se configuró `FOOTBALL_DATA_API_KEY` como secreto de producción, sin exponer su valor. Verificado: 12 pruebas focalizadas de proveedor/cuota/configuración, TypeScript y lint. | Con autorización del owner: commit, push, esperar deploy y consultar una fecha próxima para comprobar cobertura real. |
